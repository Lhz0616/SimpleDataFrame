...
public class KNN {
    ...
    */
    public static float classificationAccuracy(int[] actual_y_values, int[] predicted_y_values){
        ...
        float accuracy = correctCounter / actual_y_values.length;
        return accuracy;
    }
    ...
    Step 2: Split the dataset to train dataset and test dataset based on the size of test dataset given
            Suggested range of test dataset size = 0.2 - 0.4, suggested value = 0.25
    Step 3: Predict the values for test dataset based on the train dataset using KNNClassifier
    Step 4: Construct a confusion matrix based on predicted values and actual values
    Step 5: Two scenarios: (i) Binary classification problem (ii) Multi-class classification problem
            (i) If it is a binary classification problem:
                Calculate the precision, recall and F1-score values based on formula and confusion matrix,
                then return the F1-score
            (ii) If it is a multi-class classification problem:
